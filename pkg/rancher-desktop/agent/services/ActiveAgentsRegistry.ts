...
export class ActiveAgentsRegistry {
  ...
  async buildContextBlock(): Promise<string> {
    ...
    const parts = [
      ...
      ``,
      `**To wake an idle agent** (display message + trigger it to take a new turn):`,
      `\`\`\``,
      `<channel:heartbeat wake>Please run your status sweep now.</channel:heartbeat>`,
      `\`\`\``,
      ``,
      `**To interrupt a running agent** (abort its current turn, then wake it with your message):`,
      `\`\`\``,
      `<channel:heartbeat interrupt>Stop what you're doing — new priority task.</channel:heartbeat>`,
      `\`\`\``,
      ``,
      `**Rules:**`,
      `- Without a modifier, messages are display-only — the agent sees the text in its UI but is not forced to act.`,
      `- \`wake\` causes the agent to start a new execution turn with your message as input.`,
      `- \`interrupt\` stops the agent's current turn first, then wakes it.`,
      `- All channel messages are **fire-and-forget**. After sending, continue your work normally.`,
