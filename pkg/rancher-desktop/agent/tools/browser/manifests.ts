...
export const browserToolManifests: ToolManifest[] = [
  ...
  },
  {
    ...
    description: 'Search past chat conversations, browser visits, and workflow executions. Find previous discussions by keyword, list recent activity, retrieve details, or read the full transcript of a past conversation. Searches the conversation history database — use search_history for browsing URL history.',
    category:    'browser',
    schemaDef:   {
      action:   { type: 'enum', enum: ['search', 'recent', 'get', 'messages'], description: 'search: match query against conversation titles/summaries (returns each hit\'s id + threadId). recent: list recent conversations. get: retrieve one conversation\'s metadata by id or threadId. messages: read the compact message transcript of a past conversation by id or threadId.' },
      ...
      id:       { type: 'string', optional: true, description: 'Conversation ID. For get and messages actions.' },
      threadId: { type: 'string', optional: true, description: 'Thread ID. For get and messages actions (alternative to id).' },
    },
    operationTypes: ['read'],
