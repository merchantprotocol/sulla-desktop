...
export const metaToolManifests: ToolManifest[] = [
  ...
  },
  {
    ...
    description: 'Search active identity observations within one domain (human / business / world / agent) by keyword or phrase. Any-word ILIKE match ranked by phrase hit, word-match count, then certainty level (stated facts first) and recency. Use this before adding a new identity observation to check for existing similar ones.',
    category:    'observation',
    schemaDef:   {
      query:            { type: 'string', description: 'Search keyword or phrase — split into words, any-word ILIKE match against observation content.' },
      domain:           { type: 'string', optional: true, description: 'Identity domain to search: human, business, world, or agent (default human).' },
      limit:            { type: 'number', optional: true, description: 'Max results to return (default 20).' },
