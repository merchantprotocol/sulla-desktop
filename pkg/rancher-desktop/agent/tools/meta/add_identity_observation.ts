...
export class AddIdentityObservationWorker extends BaseTool {
  ...
  protected async _validatedCall(input: any): Promise<ToolResponse> {
    ...
    try {
      ...
      if (duplicate) {
        await IdentityObservationsModel.update(duplicate.id, { level, category, content, basis, source });
        return {
          successBoolean: true,
      ...
      const record = await IdentityObservationsModel.insert({ domain, level, category, content, basis, source });
      return {
        successBoolean: true,
