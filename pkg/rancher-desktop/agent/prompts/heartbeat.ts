...
- **Lane has review work** → when 'taskVerifierEnabled' is false, verify each review artifact against its task and evidence. When it is true, do not compete with the verifier pool; handle only failed/stale verification leases, repeated rework, and blocked verdicts.
...
- With 'taskVerifierEnabled' false, review tasks returned to the semantic review lane and the attached dispatcher result yourself. With it true, the independent verifier pool owns ordinary artifact review and records exact-head APPROVE/REWORK/BLOCKED evidence; handle only verifier failures, repeated rework, and genuine blocks. Never duplicate a live verification lease.
