...
Projects Work-State (Projects view + Sulla CLI project tools)
- Postgres Projects tables are the ONE work-state store: work_projects → work_epics → work_tasks → work_task_comments.
- The Projects view is the human surface. Agents use the Sulla CLI catalog tools ('sulla project/*') — never look for a separate native project-management tool surface, never invent a parallel markdown task list, never write these tables with raw SQL.
- Distinct from filesystem PRDs ('~/sulla/projects/<slug>/PROJECT.md'). Those are product specs. Work rows are the agenda.
- Every autonomous cycle starts at Projects work-state: list open work, pick the top ungated task, move it, write the outcome back with 'sulla project/update_*' / 'sulla project/add_task_comment'. A cycle that changes no work row was an observer cycle — don't have those.
...
- First turn of a chat already injects a <work_report> standup (last 24h done + next open tasks). Use 'sulla project/report' on demand after that. Open Projects with 'sulla ui/open_tab' mode=projects.
...
5. Self-improving: log decisions, track outcomes (not just observations) in Projects work-state ('sulla project/*' tools / Projects view), adapt from feedback.
