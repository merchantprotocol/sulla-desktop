...
export class WorkItemsModel {
  ...
  // ──────────────────────────────────────────────
  static async verifySchema(): Promise<void> {
    const required = [
      WorkItemsModel.PROJECTS,
      WorkItemsModel.EPICS,
      WorkItemsModel.TASKS,
      WorkItemsModel.COMMENTS,
    ];
    const rows = await postgresClient.query<{ name: string; relation: string | null }>(
    ...
    if (missing.length > 0) {
      throw new Error(`Projects schema is not migrated; missing: ${ missing.join(', ') }`);
    }
  }

  /** Compatibility name. This verifies capability and never executes DDL. */
  static async ensureTables(): Promise<void> {
    return WorkItemsModel.verifySchema();
  }
  ...
  static async updateTask(id: string, changes: UpdateTaskInput): Promise<WorkTaskRecord | null> {
    ...
    const updateSql = `UPDATE ${ WorkItemsModel.TASKS } SET ${ setClauses.join(', ') }
      WHERE id = $${ idx } AND status = $${ idx + 1 } RETURNING *`;
    ...
    if (changes.status !== undefined || changesSchedule) {
      updated = await postgresClient.transaction(async(client) => {
        ...
        if (!current.rows[0]) return null;
        if (current.rows[0].status !== existing.status) {
          throw new Error(`Concurrent task transition detected for ${ id }; reload and retry.`);
        }
