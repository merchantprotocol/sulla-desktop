...
export abstract class BaseNode<T extends BaseThreadState = BaseThreadState> {
  ...
  protected async wsChatMessage(
    ...
    // Post-turn subconscious writers run after the parent graph has closed.
    // Suppress their provider activity from the user-facing parent run.
    if ((state.metadata as any).subconsciousSilent) {
      return true;
    }
