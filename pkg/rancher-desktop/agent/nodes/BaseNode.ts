...
export abstract class BaseNode<T extends BaseThreadState = BaseThreadState> {
  ...
  protected stripInjectedContextBlocks(state: BaseThreadState): void {
    const BLOCK_RE = /\n*<(episodic_context|recall_context|observation_context|conversation_recall_context|unstuck_context|routine_digest)>[\s\S]*?<\/\1>/g;
    const MARKER_RE = /<(?:episodic_context|recall_context|observation_context|conversation_recall_context|unstuck_context|routine_digest)>/;
