...
export abstract class BaseNode<T extends BaseThreadState = BaseThreadState> {
  ...
  protected stripInjectedContextBlocks(state: BaseThreadState): void {
    const BLOCK_RE = /\n*<(observation_context|user_observations|self_observations|business_observations|world_observations|environment_observations|projects_observations|conversation_context|routine_digest|lane_health)>[\s\S]*?<\/\1>/g;
    const MARKER_RE = /<(?:observation_context|user_observations|self_observations|business_observations|world_observations|environment_observations|projects_observations|conversation_context|routine_digest|lane_health)>/;
