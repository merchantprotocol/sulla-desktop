...
export async function onMainProxyLoad(ipcMainProxy: any) {
  ...
  ipcMainProxy.handle('browser-context-menu:action', async(_event: Electron.IpcMainInvokeEvent, tabId: string, action: string, payload?: Record<string, unknown>) => {
    const wc = tabViewManager.getWebContents(tabId);
    ...
    case 'redo':
      ...
      break;
    case 'copy-image':
      wc.copyImageAt(payload?.x as number ?? 0, payload?.y as number ?? 0);
      break;
    case 'save-image': {
      const { dialog } = await import('electron');
      const mainWin = window.getWindow('main-agent');
      const result = await dialog.showSaveDialog(mainWin!, {
        defaultPath: payload?.suggestedFilename as string || 'image.png',
      });

      if (!result.canceled && result.filePath) {
        wc.downloadURL(payload?.srcURL as string);
      }
      break;
    }
    case 'inspect':
      wc.inspectElement(payload?.x as number ?? 0, payload?.y as number ?? 0);
      break;
    case 'view-source':
    ...
    case 'replace-selection':
      if (payload?.text) {
        wc.replace(payload.text as string);
      }
      break;
    case 'add-to-dictionary':
      if (payload?.word) {
        wc.session.addWordToSpellCheckerDictionary(payload.word as string);
      }
      break;
    }
  });
