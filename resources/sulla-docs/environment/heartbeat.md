...
4. Build a system prompt that includes:
   ...
   - Projects work-state standup + goals (injected `<work_report>` from Postgres Projects tables; environment brief, observations)
   - Directive to work autonomously from Projects work-state through `sulla project/*` — not from `~/sulla/ledger/`
