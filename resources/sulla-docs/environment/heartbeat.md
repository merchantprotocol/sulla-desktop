...
4. Build a system prompt that includes:
   ...
   - Projects work-state standup + goals (injected `<work_report>` from Postgres work tables; environment brief, observations)
   - Directive to work autonomously from Projects work-state through `sulla work/*` — not from `~/sulla/ledger/`
